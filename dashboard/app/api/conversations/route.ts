...
export async function GET(req: NextRequest) {
  try {
    ...
    if (error instanceof Error) {
      return NextResponse.json({ error: `Failed to fetch conversations: ${error.message}` }, { status: 500 })
    }

    return NextResponse.json({ error: "An unexpected error occurred while fetching conversations" }, { status: 500 })
  }
}
