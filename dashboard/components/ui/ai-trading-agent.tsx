  ...
  const fetchPrices = async () => {
    ...
    try {
      ...
      const jupiterResponse = await fetch(
        `/api/jupiter?inputMint=${fromToken.mint}&outputMint=${toToken.mint}&amount=${amount}`,
      )
      ...
      const raydiumResponse = await fetch(
        `/api/raydium?inputMint=${fromToken.mint}&outputMint=${toToken.mint}&amount=${amount}`,
      )
  ...
  return (
    <Card className="w-full max-w-md mx-auto">
      ...
      <CardContent>
        ...
        {jupiterPrice !== null && raydiumPrice !== null && (
          <div className="mt-4 space-y-2">
            <p>
              Jupiter Price: {jupiterPrice.toFixed(6)} {toToken?.symbol}
            </p>
            <p>
              Raydium Price: {raydiumPrice.toFixed(6)} {toToken?.symbol}
            </p>
          </div>
        )}
      </CardContent>
      <CardFooter className="flex flex-col space-y-2">
        ...
        <Button
          onClick={handleTrade}
          disabled={isLoading || !fromToken || !toToken || !amount || jupiterPrice === null || raydiumPrice === null}
          className="w-full"
        >
